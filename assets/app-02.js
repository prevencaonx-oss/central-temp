...
function setPage(p){
 state.page=p;buildNav();const [t,s]=pageMeta(p);document.getElementById("pageTitle").textContent=t;document.getElementById("pageSub").textContent=s;renderActions();renderPage();
}
window.go=setPage;
...
function renderActions(){
 ...
 if((state.page==="network"||state.page==="store")&&can("reports.view"))h.innerHTML=`<button class="btn ghost" onclick="printReport('${currentScopeStore()||""}')">Exportar relatório</button>${can("readings.create")?'<button class="btn topCollectBtn" onclick="goToCollectForm()">Registrar coleta</button>':""}`;
...
async function boot(){
 ...
 state.session=null;state.profile=null;state.activeStoreId=null;
 document.getElementById("loginPage").classList.remove("hidden");
 document.getElementById("app").classList.add("hidden");
 showLoad(false);
}
async function enterApp(){
 await fetchAll();document.getElementById("loginPage").classList.add("hidden");document.getElementById("app").classList.remove("hidden");
 ...
 const first=state.profile.role==="agent"?"readings":(navItems()[0]?.[0]||"readings");setPage(first);showLoad(false);
}
...
window.confirmAdminEntry=async()=>{state.activeStoreId=document.getElementById("adminEntryStore").value||null;closeModal();await enterApp()};
